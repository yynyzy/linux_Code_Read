...
! now we want to move to protected mode ... 即关闭中断
	cli			
...
end_move:
	...
	lgdt	gdt_48		
	!把后面的值（gdt_48）放在 gdtr 寄存器中，gdt_48 标签
...
//中断描述符表是用来在发生中断时，CPU 拿着中断号去中断描述符表中寻找中断处理程序的地址，找到后就跳到相应的中断程序中去执行

gdt_48:    !表示一个 48 位的数据，其中高 32 位存储着的正是全局描述符表 gdt 的内存地址
	.word	0x800		 ! gdt limit=2048, 256 GDT entries
	.word	512+gdt,0x9	 !gdt 是个标签，表示在本文件内的偏移量，而本文件是 setup.s，编译后是放在 0x90200 这个内存地址的，所以要加上 0x90200 这个值。
